...
for i in range(1,L):
    ...
    BER.append(tf.reduce_mean(tf.cast(tf.not_equal(X_IND_RESHAPED,tf.round(S2[-1])), tf.float32)))
...
for j in range(num_snr):
    ...
    for jj in range(test_iter):
        ...
        tic = tm.time()
        tmp_bers[0][jj] = batch_amp2(batch_Y, batch_H, batch_X, n0, test_batch_size, SNR1, K, N)
        toc = tm.time()
...
for i in range(1):
    ...
    for j in range(num_snr):
        sys.stderr.write(str(num_snr)+'snr ')
