...
all parameters were optimized and trained over the 30X60 Toeplitz channel, changing the channel might require parameter tuning
...
H_const=np.genfromtxt('toplitz055.csv', dtype=None, delimiter=',')

print('BPSK fixed channel')
print(K)
print(N)
print(snrdb_low)
print(snrdb_high)
...
"""
def generate_data_iid_test(B,K,N,snr_low,snr_high,H_const):
...
def generate_data_train(B,K,N,snr_low,snr_high,H_const):
    H_=np.random.randn(B,N,K)
    x_=np.sign(np.random.rand(B,K)-0.5)
    y_=np.zeros([B,N])
    w=np.random.randn(B,N)
...
for i in range(train_iter): #num of train iter
    batch_Y, batch_H, batch_HY, batch_HH, batch_X , SNR1,x_ind= generate_data_train(train_batch_size,K,N,snr_low,snr_high,H_const)
    train_step.run(feed_dict={HY: batch_HY, HH: batch_HH, X: batch_X, X_IND:x_ind})
    if i % 1000 == 0 :
        sys.stderr.write(str(i)+' ')
        batch_Y, batch_H, batch_HY, batch_HH, batch_X ,SNR1,x_ind= generate_data_iid_test(train_batch_size,K,N,snr_low,snr_high,H_const)
        results = sess.run([LOSS[L-1],SER], {HY: batch_HY, HH: batch_HH, X: batch_X, X_IND:x_ind})
        print_string = [i]+results
        print ' '.join('%s' % x for x in print_string)


batch_Y, batch_H, batch_HY, batch_HH, batch_X, SNR1,x_ind = generate_data_iid_test(test_batch_size, K, N, 10,10,H_const)
...
for j in range(num_snr):
    for jj in range(test_iter):
        ...
        print(jj)
        batch_Y, batch_H, batch_HY, batch_HH, batch_X ,SNR1,x_ind= generate_data_iid_test(test_batch_size , K,N,snr_list[j],snr_list[j],H_const)
...
save_path = saver.save(sess, "./DetNet_HD_Fixed_BPSK/BPSK_HD_Fixed_model.ckpt")
